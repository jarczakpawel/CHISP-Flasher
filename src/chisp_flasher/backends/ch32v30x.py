...
class Backend(BackendBase):
    ...
    @staticmethod
    def _cand(chip_id: int, erase_kib: int, device_type: int = 0x17) -> dict:
        return {
    ...
    CH32V303_CANDIDATES = [
        _cand.__func__(0x30, 256),
        _cand.__func__(0x31, 256),
        _cand.__func__(0x32, 128),
        _cand.__func__(0x33, 128),
    ]
    CH32V305_CANDIDATES = [_cand.__func__(0x50, 128)]
    CH32V307_CANDIDATES = [
        _cand.__func__(0x70, 256),
        _cand.__func__(0x71, 256),
        _cand.__func__(0x73, 256),
    ]
    CH32V317_CANDIDATES = [
        _cand.__func__(0x70, 256, 0x26),
        _cand.__func__(0x73, 256, 0x26),
    ]

    chip_uart_defaults = {
        'CH32V303': dict(CH32V303_CANDIDATES[0], identify_candidates=CH32V303_CANDIDATES),
        'CH32V305': dict(CH32V305_CANDIDATES[0], identify_candidates=CH32V305_CANDIDATES),
        'CH32V307': dict(CH32V307_CANDIDATES[0], identify_candidates=CH32V307_CANDIDATES),
        'CH32V317': dict(CH32V317_CANDIDATES[0], identify_candidates=CH32V317_CANDIDATES),
    }
    ...
    def _normalize_identify_candidates(self, chip_cfg: dict, identify_candidates=None) -> list[dict]:
        ...
        return [{
            ...
            'chunk_size': int(chip_cfg.get('chunk_size') or CHUNK_SIZE),
            'preserve_user_word': bool(chip_cfg.get('preserve_user_word', True)),
    ...
    def _probe_identify_native(self, link: UsbNativeLink, identify_candidates: list[dict]):
        ...
        for cand in identify_candidates:
            try:
                parsed = link.txrx_frame(self._build_identify_pkt(cand), CMD_IDENTIFY, V307_TIMEOUTS_MS['identify'])
    ...
    def supported_protocol_variants(self) -> list[str]:
        return [USB_NATIVE_PLAIN.key, 'uart_framed']
    ...
    def read_config_uart_framed(self, chip_name: str, **kwargs):
        ...
        if result.get('cfg12_hex'):
            result.update(decode_config_fields('fv30x', bytes.fromhex(result['cfg12_hex'])))
        return result

    def write_config_uart_framed(self, chip_name: str, config, **kwargs):
        result = self._uart_helper().write_config_chip(chip_name=chip_name, config=config, option_profile='fv30x', **self._uart_detect_defaults(chip_name), **kwargs)
        if result.get('cfg12_hex'):
            result.update(decode_config_fields('fv30x', bytes.fromhex(result['cfg12_hex'])))
        return result
    ...
    def detect_native_usb(self, chip_name: str, *, usb_selector: str, usb_interface_number=None, usb_endpoint_out=None, usb_endpoint_in=None, trace: bool = False, log_cb: LogFn = None, identify_candidates=None, **_kwargs) -> dict:
        ...
        try:
            ...
            return {
                ...
                'inferred_path': chip_name != 'CH32V307',
            }
        finally:
            link.close()
    ...
    def read_config_native_usb(self, chip_name: str, *, usb_selector: str, usb_interface_number=None, usb_endpoint_out=None, usb_endpoint_in=None, trace: bool = False, log_cb: LogFn = None, identify_candidates=None, **_kwargs) -> dict:
        ...
        try:
            link.flush()
            parsed, matched_ident = self._probe_identify_native(link, identify_candidates)
            code, cfg_raw = link.txrx(build_read_cfg(), CMD_READ_CFG, V307_TIMEOUTS_MS['read_cfg'])
            if code != 0x00:
            ...
            result.update(decode_config_fields('fv30x', opt.cfg12))
            return result
        finally:
            link.close()
    ...
    def write_config_native_usb(self, chip_name: str, *, usb_selector: str, usb_interface_number=None, usb_endpoint_out=None, usb_endpoint_in=None, config, trace: bool = False, log_cb: LogFn = None, identify_candidates=None, **_kwargs) -> dict:
        ...
        try:
            ...
            from chisp_flasher.protocol.config_codec import apply_config_fields
            prepared = apply_config_fields('fv30x', opt_before.cfg12, config)
            ...
            code, _ = link.txrx(build_write_cfg(data=prepared.cfg12), CMD_WRITE_CFG, V307_TIMEOUTS_MS['write_cfg'])
            if code != 0x00:
                raise BackendError('write_config native usb failed')
            self.log(log_cb, 'INFO', f'write_cfg single cfg12={prepared.cfg12.hex()}')

            code, cfg_raw_after = link.txrx(build_read_cfg(), CMD_READ_CFG, V307_TIMEOUTS_MS['read_cfg'])
            if code != 0x00:
            ...
            result.update(decode_config_fields('fv30x', opt_after.cfg12))
            return result
        finally:
            link.close()
    ...
    def erase_native_usb(self, chip_name: str, *, usb_selector: str, usb_interface_number=None, usb_endpoint_out=None, usb_endpoint_in=None, trace: bool = False, log_cb: LogFn = None, progress_cb: ProgressFn = None, seed_len: int = 49, seed_random: bool = False, identify_candidates=None, **_kwargs) -> dict:
        ...
        try:
            ...
            opt = parse_read_cfg_response(cfg_raw)
            self._log_cfg(log_cb, 'cfg_initial', opt)
            xor_key, boot_sum, key_src = self._unlock_with_seed(link, opt, chip_id, seed, label='erase', timeout_ms=V307_TIMEOUTS_MS['isp_key'])
            ...
            code, _ = link.txrx(build_erase(erase_sectors), CMD_ERASE, V307_TIMEOUTS_MS['erase'])
            if code != 0x00:
            ...
            code, _ = link.txrx(build_verify(tail_addr, 0x00, ff_enc), CMD_VERIFY, V307_TIMEOUTS_MS['verify'])
            if code != 0x00:
                raise BackendError(f'erase incomplete (tail not erased) addr=0x{tail_addr:08x}')
            try:
                link.txrx(build_isp_end(0), CMD_ISP_END, V307_TIMEOUTS_MS['isp_end'])
            except Exception:
                pass
    ...
    def verify_native_usb(self, chip_name: str, firmware_path: str, *, usb_selector: str, usb_interface_number=None, usb_endpoint_out=None, usb_endpoint_in=None, trace: bool = False, log_cb: LogFn = None, progress_cb: ProgressFn = None, seed_len: int = 52, seed_random: bool = False, identify_candidates=None, **_kwargs) -> dict:
        ...
        try:
            ...
            opt = parse_read_cfg_response(cfg_raw)
            self._log_cfg(log_cb, 'cfg_initial', opt)
            xor_key, boot_sum, key_src = self._unlock_with_seed(link, opt, chip_id, seed, label='verify', timeout_ms=V307_TIMEOUTS_MS['isp_key'])
            ...
            for i in range(blocks):
                ...
                code, _ = link.txrx(build_verify(addr, 0x00, enc), CMD_VERIFY, V307_TIMEOUTS_MS['verify'])
                if code != 0x00:
    ...
    def flash_native_usb(
        ...
        *,
        ...
        verify: bool = True,
        seed_lens: tuple[int, int] = (49, 52),
        seed_random: bool = False,
        trace: bool = False,
        log_cb: LogFn = None,
        progress_cb: ProgressFn = None, identify_candidates=None,
    ) -> dict:
        chip_cfg = dict(self.chip_native_defaults[chip_name])
        identify_candidates = self._normalize_identify_candidates(chip_cfg, identify_candidates)
        ...
        try:
            ...
            code, _ = link.txrx(build_write_cfg(data=bytes(cfg12_write)), CMD_WRITE_CFG, V307_TIMEOUTS_MS['write_cfg'])
            if code != 0x00:
            ...
            xor_key, boot_sum_program, key_src = self._unlock_with_seed(link, opt_after, chip_id, seed_program, label='program', timeout_ms=V307_TIMEOUTS_MS['isp_key'])
            ...
            for i in range(blocks):
                ...
                plain = firmware_padded[addr:addr + CHUNK_SIZE]
                enc = xor_crypt(plain, xor_key)
                code, _ = link.txrx(build_program(addr, 0x00, enc), CMD_PROGRAM, V307_TIMEOUTS_MS['program'])
                if code != 0x00:
            ...
            code, _ = link.txrx(build_program(flush_addr, 0x00, b''), CMD_PROGRAM, V307_TIMEOUTS_MS['program'])
            if code != 0x00:
            ...
            if verify:
                _, boot_sum_verify, key_src_verify = self._unlock_with_seed(link, opt_after, chip_id, seed_verify, label='verify', timeout_ms=V307_TIMEOUTS_MS['isp_key'])
